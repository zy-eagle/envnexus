...
/** POST multipart/form-data (do not set Content-Type; the browser sets the boundary). */
export async function postFormData<T>(endpoint: string, formData: FormData, options?: RequestOptions): Promise<T> {
  const headers: Record<string, string> = {};
  const token = getToken();
  if (token) {
    headers["Authorization"] = `Bearer ${token}`;
