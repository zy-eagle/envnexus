...
export default function DeviceGroupsPage({ params }: { params: { tenantId: string } }) {
  ...
  const fetchAllDevices = useCallback(async () => {
    try {
      const data = await api.get<Device[]>(`/tenants/${params.tenantId}/devices`);
  ...
  const fetchMembers = useCallback(async (groupId: string) => {
    ...
    try {
      const data = await api.get<{ items: GroupMember[] }>(`/tenants/${params.tenantId}/device-groups/${groupId}/members`);
