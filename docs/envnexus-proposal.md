...
1. `bootstrap_package`（零编译流式注入模式 - **当前默认产品级实现**）
   - **技术原理**：基于文件末尾追加（EOF Injection）技术。预先在 MinIO 存储通用的基础二进制包（Base Package）。
   - **生成过程**：当管理员请求生成下载链接时，`job-runner` 会在内存中将租户的配置（`tenant_id`, `platform_url`, `ws_url`, `enrollment_token`）序列化为 JSON，并附带魔数 `ENX_CONF_START:`，通过 `io.MultiReader` 零内存拷贝直接流式追加到基础包末尾。
   - **客户端解析**：客户端（`agent-core`）启动时，调用 `os.Executable()` 读取自身文件末尾，解析出注入的 JSON 配置，实现**免填参数的静默激活与租户绑定**。
   - **产品优势**：无需为每个租户启动复杂的 CI 交叉编译环境，生成速度达到**毫秒级**，完美实现“千人千面”的专属客户端秒级分发。
...
- 首版已完全实现 `bootstrap_package` 的 EOF 二进制注入
- `branded_package` 在首版已保留构建协议和字段，不强制实现真实重新打包
